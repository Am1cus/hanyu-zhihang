<template>
  <div>
    <div class="page-heading">
      <div><div class="eyebrow">Warning log / 03</div><h1>预警记录</h1><p>查看触发条件，记录处理结果。当前预警由遥测阈值触发。</p></div>
    ...
    <div class="warning-summary"><div><strong>{{ system.unhandledCount }}</strong><span>条待处理预警</span></div><p>温度 ≤ −25°C · SOC ≤ 20% · 风速 ≥ 8 m/s<br><span>同一设备同类预警冷却60秒；这不是 AI 故障分类。</span></p><el-button @click="showPending">只看待处理</el-button></div>
    ...
    <section class="panel">
      ...
      <div class="table-wrap">
        <el-table :data="warningList" v-loading="loading" row-key="id" empty-text="当前筛选下没有预警记录">
          <el-table-column type="expand"><template #default="{ row }"><div class="warning-detail"><p>{{ row.message }}</p><p>触发值：温度 {{ format(row.triggerTemperature, 2) }} °C / SOC {{ format(row.triggerBatteryLevel, 2) }} % / 风速 {{ format(row.triggerWindSpeed, 2) }} m/s</p><p>处理说明：{{ row.handleResult || '尚未记录' }}</p><p>处理时间：{{ dateTime(row.handleTime) }}</p></div></template></el-table-column>
          ...
          <el-table-column label="操作" width="125" fixed="right"><template #default="{ row }"><el-button v-if="row.handleStatus === 0" size="small" link type="primary" @click="openHandle(row)">记录处理结果</el-button><span v-else class="muted" style="font-size:11px">展开查看说明</span></template></el-table-column>
        </el-table>
      </div>
...
<style scoped>
.warning-summary{display:flex;align-items:center;gap:32px;border:1px solid #e4dfcc;background:#f3f0e4;border-radius:9px;padding:24px;margin-bottom:24px}.warning-summary>div{display:flex;align-items:baseline;gap:10px;white-space:nowrap}.warning-summary strong{font-size:36px;font-weight:500;color:#8d744a}.warning-summary span{font-size:12px;color:#8e8975}.warning-summary p{font-size:12px;color:#797662;line-height:2}.warning-summary p span{font-size:11px;color:#9a9686}.warning-summary .el-button{margin-left:auto}.filter-bar{display:flex;align-items:end;gap:12px;flex-wrap:wrap;padding:22px}.filter-bar label{display:grid;gap:8px;font-size:11px;color:var(--muted)}select{height:32px;min-width:115px;padding:0 28px 0 10px;border:1px solid var(--line);border-radius:5px;background:white;color:var(--ink);font-size:12px}.auto-refresh{margin-left:auto;font-size:10px;color:#929987;line-height:32px}.table-wrap{padding:0 22px}.time-cell{font-size:11px;font-variant-numeric:tabular-nums}.pending-status{color:#a16b33;font-size:12px}.critical{color:#a34733;background:#fff0e9}.warning-detail{padding:15px 45px;color:var(--muted);font-size:12px;line-height:2}.pagination-bar{display:flex;align-items:center;justify-content:space-between;gap:15px;padding:22px}.pagination-bar>span{font-size:10px;color:#929987}.dialog-event{font-size:13px;margin-bottom:24px;color:var(--muted)}
